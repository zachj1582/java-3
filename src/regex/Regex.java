...
class Regex {
    public static void main(String[] args) {
        ...
        System.out.println("The following pattern ('\\d'){36} matches : ... any number that is 36 digits in length");
        ...
        for(int i = 0; i < TEKmentors.length; i++){
            Pattern firstName = Pattern.compile("\\w+\\s");
            Matcher matcher = firstName.matcher(TEKmentors[i]);
            while (matcher.find()) {
                tekMentors[i] = matcher.group();
                System.out.println("Tek: " + tekMentors[i]);
            }
        }
        ...
        Pattern bookPattern = Pattern.compile("book|Book");
        for(int i = 0; i<words.length;i++){
            Matcher matcher = bookPattern.matcher(words[i]);
            if(matcher.find()){
                bookCount += 1;
            }
        }
        ...
        String[] sleepyArray = sleepy.split("sleepy");
        for(String element: sleepyArray){
            System.out.println(element);
        }

        // 4.b combine the array that you just created into a string
        String not_sleepy = String.join("", sleepyArray); //punctuation marks will be here


        //4.c remove the punctuation marks from the notSleepy string.
        String[] noPunct = not_sleepy.split("\\W");
        String notSleepy = String.join("", noPunct); //no punctuation marks should be here.
        System.out.println(notSleepy);
        //4.d Now replace all the occurences of 'sleepy' with the word 'happy'.  Call the new string happy. 
        String[] meh = sleepy.split("sleepy");
        String happy = String.join("happy", meh);
        System.out.println("HAPPY STRING: " + happy);
